...
class Export:
    ...
    def is_gold(self, cand):
        ...
        """
        if cand.id in self.gold:
            return 1
        else:
            return 0

    def create_gold_function(self, session, candidates):
        def _approximate_offset(document, relevant_candidates):
            entety_1_start = document.entetys[0].start_offset
            ...
            for candidate in relevant_candidates:
                candidate_row = (
                    ...
                    candidate[0].context.get_span(),
                    candidate[0].context.sentence.id,
                    candidate[1].document_id,
                    candidate[1].context.get_span(),
                    candidate[1].context.sentence.id,
                )
            ...
            if not table:
                logging.warning(f'No offsets found: "{document.filename}"')
                return None
            else:
                row = sorted(table, key=lambda tup: tup[-1])[0]
                return row[0]
        ...
        document_candidate = defaultdict(list)
        for candidate in candidates[0]:
            document_candidate[candidate.document_id].append(candidate)

        for document in self.export_documents:
            if len(document.entetys) != 2:
                continue

            relevant_candidates = document_candidate[document.document_id]
            gold_candidate = _approximate_offset(document, relevant_candidates)
            if gold_candidate:
                self.gold.append(gold_candidate)
